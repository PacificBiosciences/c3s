...
public:
    ...
    virtual bool parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes) = 0;
...
public:
    ...
    bool parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes) override;

    friend std::unique_ptr<Parser<T>> createParser<bioparser::FastaParser, T>(
...
public:
    ...
    friend std::unique_ptr<Parser<T>> createParser<bioparser::FastqParser, T>(
        const std::string& path);
...
public:
    ...
    friend std::unique_ptr<Parser<T>> createParser<bioparser::MhapParser, T>(
        const std::string& path);
...
public:
    ...
    friend std::unique_ptr<Parser<T>> createParser<bioparser::PafParser, T>(
        const std::string& path);
...
public:
    ...
    friend std::unique_ptr<Parser<T>> createParser<bioparser::SamParser, T>(
        const std::string& path);
...
Parser<T>::Parser(gzFile input_file, uint32_t storage_size)
    : input_file_(input_file, gzclose), buffer_(kBufferSize, 0), storage_(storage_size, 0)
{}
...
template <class T>
bool Parser<T>::parse_objects(std::vector<std::shared_ptr<T>>& dst, uint64_t max_bytes)
{
...
template <class T>
FastaParser<T>::FastaParser(gzFile input_file) : Parser<T>(input_file, kSSS + kMSS)
{}
...
template <class T>
bool FastaParser<T>::parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes)
{
    ...
    while (!is_end) {
        ...
        uint64_t read_bytes =
            gzfread(this->buffer_.data(), sizeof(char), this->buffer_.size(), input_file);
        ...
        for (uint32_t i = 0; i < read_bytes; ++i) {
            ...
            if (is_valid) {
                ...
                dst.emplace_back(std::unique_ptr<T>(new T((const char*)&(name[1]), name_length - 1,
...
template <class T>
FastqParser<T>::FastqParser(gzFile input_file) : Parser<T>(input_file, kSSS + 2 * kMSS)
{}
...
template <class T>
bool FastqParser<T>::parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes)
{
...
template <class T>
MhapParser<T>::MhapParser(gzFile input_file) : Parser<T>(input_file, kSSS)
{}
...
template <class T>
bool MhapParser<T>::parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes)
{
    ...
    uint32_t a_rc = 0, a_begin = 0, a_end = 0, a_length = 0, b_rc = 0, b_begin = 0, b_end = 0,
             b_length = 0, minmers = 0;
    double error = 0;
    ...
    while (!is_end) {
        ...
        for (uint32_t i = 0; i < read_bytes; ++i) {
            ...
            if (c == '\n') {
                ...
                dst.emplace_back(
                    std::unique_ptr<T>(new T(a_id, b_id, error, minmers, a_rc, a_begin, a_end,
                                             a_length, b_rc, b_begin, b_end, b_length)));
...
template <class T>
PafParser<T>::PafParser(gzFile input_file) : Parser<T>(input_file, 3 * kSSS)
{}
...
template <class T>
bool PafParser<T>::parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes)
{
    ...
    uint32_t q_name_length = 0, q_length = 0, q_begin = 0, q_end = 0, t_name_length = 0,
             t_length = 0, t_begin = 0, t_end = 0, matching_bases = 0, overlap_length = 0,
             mapping_quality = 0;
    char orientation = '\0';
    ...
    while (!is_end) {
        ...
        for (uint32_t i = 0; i < read_bytes; ++i) {
            ...
            if (c == '\n') {
                ...
                dst.emplace_back(std::unique_ptr<T>(new T(q_name, q_name_length, q_length, q_begin,
                                                          q_end, orientation, t_name, t_name_length,
                                                          t_length, t_begin, t_end, matching_bases,
                                                          overlap_length, mapping_quality)));
...
template <class T>
SamParser<T>::SamParser(gzFile input_file) : Parser<T>(input_file, 5 * kSSS + 2 * kMSS)
{}
...
template <class T>
bool SamParser<T>::parse_objects(std::vector<std::unique_ptr<T>>& dst, uint64_t max_bytes)
{
    ...
    const char *q_name = nullptr, *t_name = nullptr, *cigar = nullptr, *t_next_name = nullptr,
               *sequence = nullptr, *quality = nullptr;

    uint32_t q_name_length = 0, flag = 0, t_name_length = 0, t_begin = 0, mapping_quality = 0,
             cigar_length = 0, t_next_name_length = 0, t_next_begin = 0, template_length = 0,
    ...
    while (!is_end) {
        ...
        for (uint32_t i = 0; i < read_bytes; ++i) {
            ...
            if (c == '\n') {
                ...
                if (q_name_length == 0 || t_name_length == 0 || cigar_length == 0 ||
                    t_next_name_length == 0 || sequence_length == 0 || quality_length == 0 ||
                ...
                dst.emplace_back(std::unique_ptr<T>(new T(
                    q_name, q_name_length, flag, t_name, t_name_length, t_begin, mapping_quality,
                    cigar, cigar_length, t_next_name, t_next_name_length, t_next_begin,
                    template_length, sequence, sequence_length, quality, quality_length)));
