...
std::unique_ptr<Edge> Graph::createEdge(uint32_t begin_node_id, uint32_t end_node_id,
                                        uint32_t label, uint32_t weight)
{

    return std::unique_ptr<Edge>(new Edge(begin_node_id, end_node_id, label, weight));
}

Edge::Edge(uint32_t begin_node_id, uint32_t end_node_id, uint32_t label, uint32_t weight)
    : begin_node_id_(begin_node_id)
    , end_node_id_(end_node_id)
    , sequence_labels_(1, label)
    , sequence_weights_(1, weight)
...
std::unique_ptr<Graph> createGraph() { return std::unique_ptr<Graph>(new Graph()); }
...
void Graph::add_edge(uint32_t begin_node_id, uint32_t end_node_id, uint32_t weight)
{
...
void Graph::add_alignment(const Alignment& alignment, const std::string& sequence, uint32_t weight)
{

    this->add_alignment(alignment, sequence.c_str(), sequence.size(), weight);
}

void Graph::add_alignment(const Alignment& alignment, const char* sequence, uint32_t sequence_size,
                          uint32_t weight)
{
                          ...
                          const std::string& quality)
{
    ...
    this->add_alignment(alignment, sequence.c_str(), sequence.size(), quality.c_str(),
                        quality.size());
}

void Graph::add_alignment(const Alignment& alignment, const char* sequence, uint32_t sequence_size,
                          const char* quality, uint32_t quality_size)
{
                          ...
                          const std::vector<uint32_t>& weights)
{
    ...
    if (alignment.empty()) {  // no alignment
        int32_t begin_node_id = this->add_sequence(sequence, weights, 0, sequence_size);
        ++num_sequences_;
    ...
    int32_t tail_node_id =
        this->add_sequence(sequence, weights, valid_seq_ids.back() + 1, sequence_size);
    ...
    for (uint32_t i = 0; i < alignment.size(); ++i) {
        ...
        if (alignment[i].first == -1) {
            ...
            if (decoder_[nodes_[alignment[i].first]->code_] == letter) {
                ...
                if (aligned_to_node_id == -1) {
                    ...
                    nodes_[new_node_id]->aligned_nodes_ids_.emplace_back(alignment[i].first);
                    nodes_[alignment[i].first]->aligned_nodes_ids_.emplace_back(new_node_id);
        ...
        if (head_node_id != -1) {
            ...
            this->add_edge(head_node_id, new_node_id, prev_weight + weights[alignment[i].second]);
        }
    ...
    if (tail_node_id != -1) {
        // both nodes contribute to edge weight
        this->add_edge(head_node_id, tail_node_id, prev_weight + weights[valid_seq_ids.back() + 1]);
    }
                                                 ...
                                                 bool include_consensus)
{
    ...
    for (uint32_t i = 0; i < num_sequences_; ++i) {
        ...
        while (true) {
            alignment_str[msa_node_ids[curr_node_id]] = decoder_[nodes_[curr_node_id]->code_];
    ...
    if (include_consensus) {
        ...
        for (const auto& node_id : consensus_) {
            alignment_str[msa_node_ids[node_id]] = decoder_[nodes_[node_id]->code_];
        }
        dst.emplace_back(alignment_str);
    }
}

std::string Graph::generate_consensus(int minCov)
{

    this->traverse_heaviest_bundle();
    std::string consensus_str = "";
    for (const auto& node_id : consensus_) {
        if (static_cast<int>(nodes_[node_id]->coverage()) >= minCov)
...
std::string Graph::generate_consensus(std::vector<uint32_t>& dst, bool verbose)
{
    ...
    auto consensus_str = this->generate_consensus(0);
...
void Graph::traverse_heaviest_bundle()
{
    ...
    if (nodes_[max_score_id]->out_edges_.size() != 0) {
        ...
        while (nodes_[max_score_id]->out_edges_.size() != 0) {
            max_score_id =
                this->branch_completion(scores, predecessors, node_id_to_rank[max_score_id]);
        }
    }
...
uint32_t Graph::branch_completion(std::vector<int32_t>& scores, std::vector<int32_t>& predecessors,
                                  uint32_t rank)
{
                                       ...
                                       std::vector<int32_t>& subgraph_to_graph_mapping) const
{
    ...
    for (uint32_t i = 0; i < is_subgraph_node.size(); ++i) {
        ...
        for (const auto& edge : nodes_[i]->in_edges_) {
            ...
            subgraph->add_edge(graph_to_subgraph_mapping[edge->begin_node_id_], subgraph_id,
                               edge->total_weight_);
        }
...
void Graph::print_csv() const
{
    ...
    for (uint32_t i = 0; i < nodes_.size(); ++i) {
        ...
        for (const auto& edge : nodes_[i]->out_edges_) {
            printf("    %d -> %d [label = \"%d\"", i, edge->end_node_id_, edge->total_weight_);
