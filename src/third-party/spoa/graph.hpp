...
public:
    ...
    const std::vector<uint32_t>& rank_to_node_id() const { return rank_to_node_id_; }

    uint32_t num_codes() const { return num_codes_; };

    uint8_t coder(uint8_t c) const { return coder_[c]; }

    uint8_t decoder(uint8_t code) const { return decoder_[code]; }

    void add_alignment(const Alignment& alignment, const std::string& sequence,
                       uint32_t weight = 1);

    void add_alignment(const Alignment& alignment, const char* sequence, uint32_t sequence_size,
                       uint32_t weight = 1);
    ...
    std::string generate_consensus(int minCov);
...
private:
    ...
    static std::unique_ptr<Edge> createEdge(uint32_t begin_node_id, uint32_t end_node_id,
    ...
    int32_t add_sequence(const char* sequence, const std::vector<uint32_t>& weights, uint32_t begin,
                         uint32_t end);
...
public:
    ...
    const std::vector<std::shared_ptr<Edge>>& in_edges() const { return in_edges_; }

    const std::vector<std::shared_ptr<Edge>>& out_edges() const { return out_edges_; }

    const std::vector<uint32_t>& aligned_nodes_ids() const { return aligned_nodes_ids_; }
...
public:
    ...
    uint32_t begin_node_id() const { return begin_node_id_; }

    uint32_t end_node_id() const { return end_node_id_; }

    friend Graph;
    friend Node;

private:
    Edge(uint32_t begin_node_id, uint32_t end_node_id, uint32_t label, uint32_t weight);
